...
const screenshotPath = argValue("--screenshot") || "/tmp/hermes-damocles-smoke.png";
...
async function launchChrome(url) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), "hermes-damocles-smoke-"));
...
async function runSmoke(cdp) {
  ...
  await clickAndExpect(cdp, `clickNav("Report")`, "Damocles Report", "report route");
  await clickAndExpect(cdp, `clickNav("Guideposts")`, "What deserves your attention", "guideposts route");
  await clickAndExpect(cdp, `clickNav("Damocles")`, "Journey Index", "damocles route");
