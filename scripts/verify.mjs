...
const checks = [
  ...
  [pythonCmd, ["-m", "py_compile", "dashboard/plugin_api.py"]],
  [pythonCmd, ["scripts/test-plugin-api.py"]],
  [pythonCmd, ["-m", "html.parser", "index.html"]],
  [pythonCmd, ["-m", "html.parser", "Hermes Damocles _standalone_.html"]],
];
...
for (const file of ["dashboard/dist/index.js", "index.html", "Hermes Damocles _standalone_.html"]) {
...
if (!/dashboard\/dist\/damocles\.css\?v=[0-9a-f]{12}/.test(indexHtml)) {
  console.error("index.html must cache-bust dashboard/dist/damocles.css");
  process.exit(1);
}
